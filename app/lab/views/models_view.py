...
class InstanceSelector(QComboBox):
    ...
    def refresh(self, instances: list[tuple[int, RemoteGGUF]], connected_iids: set[int]):
        ...
        for iid, gguf in instances:
            ...
            self.addItem(f"Instance #{iid} — {tag}", iid)
...
class ConfigurePanel(GlassCard):
    ...
    def __init__(self, store, parent=None):
        ...
        self._form: ModelConfigForm | None = None

        self.setMinimumWidth(356)
        ...
        self._lay.addWidget(inst_lbl)

        self._instance_combo = InstanceSelector()
        self._instance_combo.currentIndexChanged.connect(self._on_instance_changed)
        self._lay.addWidget(self._instance_combo)

        # Form container
        self._form_container = QWidget()
        form_lay = QVBoxLayout(self._form_container)
        form_lay.setContentsMargins(0, 0, 0, 0)
        form_lay.setSpacing(t.SPACE_3)
        self._lay.addWidget(self._form_container, 1)
    ...
    def set_model(self, filename: str, path: str, instances: list[tuple[int, RemoteGGUF]],
        ...
        # Refresh instance picker
        self._instance_combo.refresh(instances, connected_iids)
        ...
        # Rebuild form
        while self._form_container.layout().count():
            item = self._form_container.layout().takeAt(0)
            if item.widget():
                item.widget().deleteLater()

        iid = self._instance_combo.currentData() or default_iid
        st = self.store.get_state(iid) if iid else None
        initial_p = st.model_configs.get(path) if st else None
        self._form = ModelConfigForm(path, self.store, initial_params=initial_p, target_iid=iid)
        self._form_container.layout().addWidget(self._form)
    ...
    def _on_instance_changed(self, index: int):
        ...
        if self._form and iid is not None:
            self._form._target_iid = iid
            self._form._update_preview()

    def _on_save(self):
        if self._form and self._current_iid is not None and self._current_path is not None:
            params = self._form.gather_params()
            self.save_requested.emit(params, self._current_iid, self._current_path)

    def _on_launch(self):
        if self._form and self._current_iid is not None:
            params = self._form.gather_params()
...
class ModelsView(QWidget):
    ...
    def _build_model_card(self, filename: str, instances: list[tuple[int, RemoteGGUF]]) -> GlassCard:
        ...
        configure_btn.setMinimumWidth(130)
        ...
        def on_configure():
            ...
            self.configure_panel.setVisible(True)
            # Adjust splitter to show panel
