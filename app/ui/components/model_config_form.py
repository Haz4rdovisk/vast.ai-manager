"""Reusable model configuration form — now with Save functionality."""
...
class ModelConfigForm(QWidget):
    ...
    def __init__(self, model_path: str, store, initial_params: ServerParams = None, parent=None):
        ...
        grid_frame = QFrame()
        grid_frame.setStyleSheet(f"background: {t.SURFACE_3}; border-radius: {t.RADIUS_MD}px;")
        ...
        r1 = QHBoxLayout()
        r1.addWidget(self._labeled_control("Context Size:", self._init_spin(128, 131072, self._params.context_length, 1024, "ctx")))
        r1.addWidget(self._labeled_control("GPU Layers:", self._init_spin(0, 999, self._params.gpu_layers, 1, "ngl")))
        grid.addLayout(r1)
        ...
        r3 = QHBoxLayout()
        r3.addWidget(self._labeled_control("Parallel Req:", self._init_spin(1, 16, self._params.parallel_requests, 1, "np")))
        ...
        self.rp_spin.setRange(0.0, 3.0); self.rp_spin.setValue(self._params.repeat_penalty); self.rp_spin.setSingleStep(0.1)
        ...
        self.preview.setStyleSheet(f"background: {t.SURFACE_1}; color: {t.TEXT_HI}; font-family: 'Consolas'; font-size: 9pt; border: none;")
        lay.addWidget(self.preview)

        # 4. Action Row (Repurposed to Save)
        actions = QHBoxLayout()
    ...
    def _labeled_control(self, label, control) -> QWidget:
        ...
        l.setSpacing(2)
        lbl = QLabel(label)
        lbl.setProperty("role", "small-muted")
        l.addWidget(lbl)
        l.addWidget(control)
        return w
    ...
    def _update_preview(self, *_):
        ...
        st = self._store.get_state(self._store.selected_instance_id) if self._store.selected_instance_id else None
        binary = (st.setup.llamacpp_path if st else "") or "/opt/llama.cpp/build/bin/llama-server"
